#!/usr/bin/env python3
"""
Test script for DocumentDatabaseManager's search_documents method.
This script tests the search functionality in isolation without the GUI.
"""
...
def test_search(query, source_name=None, limit=20):
    """
    Test the search_documents method with the given parameters.
    ...
    """
    print(f"\n{'='*80}")
    print(f"Testing search with query: '{query}', source: {source_name}, limit: {limit}")
    print(f"{'='*80}")
...
def main():
    ...
    test_search("vaccine efficacy")
    
    # Test with a source filter
    test_search("covid", source_name="pubmed")
    test_search("covid", source_name="medrxiv")
    
    # Test with a complex query
    test_search("SARS-CoV-2 AND vaccine AND efficacy")
    
    # Test with a query that might be slow
    test_search("the")
    
    # Test with a query containing special characters
    test_search("covid-19 & vaccine")
    
    # Test with a query that should return no results
    test_search("xyzabc123notfound")
