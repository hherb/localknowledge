"""
Fixed text chunker that respects minimum chunk size and sentence boundaries.

This module provides an improved text chunker that:
1. Respects a minimum chunk size
2. Ensures chunks don't break in the middle of words
3. Tries to break at sentence boundaries
...
class FixedTextChunker(BaseChunker):
    """
    Fixed text chunker that respects minimum chunk size and sentence boundaries.
    """
    ...
    def __init__(self, 
                 ...
                 **kwargs):
        """
        Initialize the fixed text chunker.
    ...
    def chunk(self, text: str, metadata: Optional[Dict[str, Any]] = None, **kwargs) -> List[Chunk]:
        """
        Split text into chunks based on character count, respecting minimum chunk size and sentence boundaries.
