...
def generate_and_store_embeddings(
    ...
    try:
        ...
        # Prepare queries with proper parameters
        id_condition = ''
        params = [embed_source_id]
        if last_processed_id is not None:
            id_condition = 'AND d.id > %s'
            ...
            with conn.cursor() as count_cur:
                count_query = """
                    ...
                    WHERE d.abstract IS NOT NULL
                        ...
                        %s
                """
                count_cur.execute(count_query % (id_condition), tuple(params))
        ...
        # Prepare the cursor for fetching documents
        fetch_query = """
        ...
        # Only add the ID condition if we're resuming
        if last_processed_id is not None:
            fetch_query += " AND d.id > %s"
        
        # Add sorting
        fetch_query += " ORDER BY d.id"
        ...
        with tqdm(total=total_docs, 
            ...
            try:
                with conn.cursor(name='doc_cursor', cursor_factory=DictCursor) as fetch_cur:
                    ...
                    # Execute with appropriate parameters based on whether we're resuming
                    if last_processed_id is not None:
                        fetch_cur.execute(fetch_query, tuple(params))
                    else:
