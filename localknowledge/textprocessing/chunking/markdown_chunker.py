...
based on headings and subheadings.
"""
...
class MarkdownChunker(BaseChunker):
    ...
    def __init__(self,
        ...
        # Fallback chunker for large sections
        self.text_chunker = TextChunker(chunk_size=max_chunk_size, overlap=100)
    ...
    def chunk(self, text: str, metadata: Optional[Dict[str, Any]] = None, **kwargs) -> List[Chunk]:
        ...
        # If no headings found, use the text chunker
        if not headings:
            return self.text_chunker.chunk(text, metadata=metadata, chunk_size=max_chunk_size)
        ...
        for i, heading in enumerate(heading_info):
            ...
            if len(section_text) > max_chunk_size:
                ...
                sub_chunks = self.text_chunker.chunk(section_text, metadata=sub_metadata)

                # Add heading information to each sub-chunk
                for j, sub_chunk in enumerate(sub_chunks):
                    # Update the chunk type
                    sub_chunk.metadata['chunk_type'] = 'markdown_sub'
