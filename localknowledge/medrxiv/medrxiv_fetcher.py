...
class MedRxivFetcher:
    ...
    def _try_direct_text_url(self, doi):
        """
                ...
                try:
                    ...
                    try:
                        ...
                        for days_offset in range(8):  # 0 to 7 days
                            ...
                            logger.info(f"Trying date variation {days_offset}: {date_str} for DOI {doc_id}")
                            ...
                            try:
                                head_response = self.session.head(url, timeout=5)
                                if head_response.status_code == 200:
                                    logger.info(f"Found XML URL with date variation ({days_offset} days offset from {base_date.strftime('%Y-%m-%d')} to {check_date.strftime('%Y-%m-%d')}): {url}")
                                    return url
                            except requests.RequestException:
    ...
    def _get_format_urls(self, preprint_url):
        ...
        """
        format_urls = {'pdf': None, 'html': None, 'xml': None, 'txt': None}
        ...
        # If we couldn't get formats from direct methods or don't have a DOI, fall back to scraping
        try:
            ...
            # Find plain text link - may be labeled as "Full Text (Plain)" or similar
            if not format_urls['txt']:
                txt_link = soup.select_one('a[href*=".full.txt"]')
                if txt_link:
                    format_urls['txt'] = urljoin(self.BASE_URL, txt_link['href'])
    ...
    def _get_metadata_from_api_direct(self, doi):
        ...
        try:
            ...
            if 'collection' in data and len(data['collection']) > 0:
                ...
                try:
                    # Use our enhanced direct URL method which includes date variations
                    direct_xml_url = self._try_direct_xml_url(doi)
                    if direct_xml_url:
                        #logger.info(f"Found XML URL using enhanced fallback mechanism: {direct_xml_url}")
                        return {'jatsxml': direct_xml_url}
    ...
    def download_preprint(self, doi_or_url, formats=None, filename_prefix=None):
        ...
        Args:
            ...
            formats (list): List of formats to download ('pdf', 'html', 'xml', 'txt')
    ...
    def check_format_availability(self, doi_or_url):
        ...
        availability = {
            ...
            'txt': format_urls['txt'] is not None
        }
...
if __name__ == "__main__":
    ...
    # Test with specific DOIs known to have plain text
    test_doi = "10.1101/2024.12.23.24319533"
    print(f"\nTesting specific DOI: {test_doi}")
    text_url = fetcher._try_direct_text_url(test_doi)
    print(f"Plain text URL: {text_url}")

    # Test with specific DOIs known to have XML in different locations
    test_doi2 = "10.1101/2021.08.25.21262601"
    print(f"\nTesting specific DOI: {test_doi2}")
    xml_url = fetcher._try_direct_xml_url(test_doi2)
