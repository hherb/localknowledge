...
def generate_summaries() -> int:
    ...
    """
    try:
        logger.info("Starting AI summarization for all papers without summaries")

        # Connect to the database
        db_manager = MedRxivDatabaseManager()

        # Get papers without summaries
        papers = db_manager.get_preprints_without_summaries(limit=None)
        if not papers:
...
def fetch_markdown_for_recent_papers(days_back: int = 7) -> int:
    """
    ...
    try:
        ...
        papers = db_manager.get_recent_preprints_without_fulltext(days_back=days_back, limit=None)
...
def run_daily_update(summarize: bool = False, convert_to_markdown: bool = True):
    """
    ...
    try:
        ...
        update_medrxiv_database(
            download_pdfs=True,  # Change to True if you want to download PDFs
            max_retries=5,
            start_date_override=None,  # Let it auto-determine from database
            days_to_fetch=90,  # Fallback in case database is empty
            end_date=yesterday
        )
        ...
        if convert_to_markdown:
            ...
            markdown_count = fetch_markdown_for_recent_papers(days_back=7)
