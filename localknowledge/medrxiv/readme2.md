...
- **XML Image Handling**: When processing papers in XML format, the tool cannot download the actual images since the XML files only contain references to images (like `xlink:href` attributes) but not the actual image URLs or data. For XML-based papers, the script will include figure captions but not the images themselves.

- **HTML Availability**: Although medRxiv states that papers are available in HTML format, many papers (especially recent ones) are only available in PDF and XML formats.

- **API Reliability**: The medRxiv API might not have indexed very recent papers. In these cases, the tool falls back to direct URL construction.

- **Website Structure Changes**: The tool may need updating if medRxiv significantly changes their website structure or API endpoints.# MedRxiv to Markdown Converter
