#!/usr/bin/env python3
"""
Test script for the new markdown chunker.
"""
...
def main():
    """Main function to test the new markdown chunker."""
...
"""
    ...
    # Now test with the Example.md file
    print("\n\nTesting with Example.md file:")
    ...
    # Chunk the example text
    example_chunks = chunker.chunk(example_text, metadata=example_metadata)
    
    # Print the chunks
    print(f"\nFound {len(example_chunks)} chunks in Example.md:\n")
    ...
    # Check for overlapping content
    print("\nChecking for overlapping content in Example.md chunks...")
    for i in range(len(example_chunks) - 1):
        chunk1 = example_chunks[i].text
        chunk2 = example_chunks[i + 1].text
