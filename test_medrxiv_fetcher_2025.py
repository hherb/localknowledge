...
Specifically testing the DOI 10.1101/2025.03.07.25323558
"""
...
if __name__ == "__main__":
    ...
    test_doi = "10.1101/2025.03.07.25323558"
    print(f"Testing DOI: {test_doi}")
    
    # First, let's directly check the URL we know works
    known_url = "https://www.medrxiv.org/content/medrxiv/early/2025/03/11/2025.03.07.25323558.source.xml"
    try:
    ...
    print(f"Result from algorithm: {xml_url}")
    
    # Let's also manually check each date variation
    doc_id = test_doi.split('/')[-1]
    year = doc_id[:4]
    month = doc_id.split('.')[1]
    day = doc_id.split('.')[2][:2]
    
    base_date = datetime(int(year), int(month), int(day))
    print(f"Base date from DOI: {base_date}")
    
    for days_offset in range(8):
        check_date = base_date + timedelta(days=days_offset)
        date_str = f"{check_date.year}/{check_date.month:02d}/{check_date.day:02d}"
        url = f"{fetcher.BASE_URL}/content/medrxiv/early/{date_str}/{doc_id}.source.xml"
        print(f"Checking date offset {days_offset}: {date_str} -> {url}")
        try:
            head_response = fetcher.session.head(url, timeout=5)
            print(f"  Status code: {head_response.status_code}")
        except Exception as e:
            print(f"  Error: {e}")
